...
SMTool.ctxDuplicateNode = function () {
    ...
    var orig = SMData.nodes.get(SMData.selectedNode);
    if (!orig) return;

    var id = SMData.nextId++;
    var node = new SpineNodeData(id);
    node.name = orig.name + ' (副本)';
    node.x = orig.x + 50;
    node.y = orig.y + 50;
    ...
    if (node._srcAtlasText && node._srcTexDataUrl &&
        ...
        SMTool._loadFromSourceData(node).then(function () {
            ...
            console.error('[Duplicate] Failed to restore rendering:', err);
        });
    }
...
SMTool.ctxDeleteNode = function () {
    if (SMData.selectedNode) SMTool.deleteNode(SMData.selectedNode);
    document.getElementById('ctxMenu').style.display = 'none';
};
...
SMTool.init = function () {
    ...
    window.addEventListener('wheel', function (e) {
        if (!e.target.closest('.state-list') && !e.target.closest('.ip-body') && !e.target.closest('#conditionEditor')) {
            e.preventDefault();
            SMTool._onWheel(e);
        }
    }, { passive: false });
    ...
    ce.addEventListener('keydown', function (e) {
        if (e.key === 'Enter') SMTool.saveCondition();
