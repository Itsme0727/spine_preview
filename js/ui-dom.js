    ...
    if (node.nodeType === 'shortText' || node.nodeType === 'textBox') {
        ...
        el.innerHTML =
            '<div class="header delayer-header" onmousedown="event.stopPropagation();SMTool._onHD(event,' + node.id + ')">' +
                '<span class="delayer-title">⏱ 模拟延时器</span>' +
                '<div class="btns">' +
                    '<button onclick="event.stopPropagation();SMTool.deleteNode(' + node.id + ')" title="删除节点">✕</button>' +
                '</div>' +
            '</div>' +
            ...
            '</div>' +
            '<span class="scale-handle" onmousedown="event.stopPropagation();SMTool._onScaleStart(event,' + node.id + ')" title="拖拽缩放"><i class="scale-handle-icon"></i></span>';
    } else if (node.nodeType === 'progDelayer') {
        el.classList.add('delayer-node', 'prog-delayer-node');
        if (node._delayValue === undefined) node._delayValue = 1.0;
        var pdelayVal = node._delayValue;
        el.innerHTML =
            '<div class="header delayer-header" onmousedown="event.stopPropagation();SMTool._onHD(event,' + node.id + ')">' +
                '<span class="delayer-title">⏱ 程序延时器</span>' +
                '<div class="btns">' +
                    '<button onclick="event.stopPropagation();SMTool.deleteNode(' + node.id + ')" title="删除节点">✕</button>' +
                '</div>' +
            '</div>' +
            '<div class="delayer-body">' +
                '<div class="delayer-set-row">' +
                    '<button class="delayer-step-btn" onclick="event.stopPropagation();SMTool._delayerStep(' + node.id + ',-0.1)">◀</button>' +
                    '<input type="number" class="delayer-input" value="' + pdelayVal + '" step="0.001" min="0.001" ' +
                        'onchange="event.stopPropagation();SMTool._delayerSet(' + node.id + ',parseFloat(this.value)||1.0)" ' +
                        'onclick="event.stopPropagation()" onkeydown="event.stopPropagation()">' +
                    '<button class="delayer-step-btn" onclick="event.stopPropagation();SMTool._delayerStep(' + node.id + ',0.1)">▶</button>' +
                    '<span class="delayer-unit">秒</span>' +
            ...
            '</div>' +
            '<div class="anim-bar" style="display:flex;justify-content:space-between">' +
                '<div class="conn-dot input" onclick="event.stopPropagation();SMTool._onDot(' + node.id + ',\'progDelayer\',\'input\')" title="连线输入"></div>' +
                '<div class="conn-dot output" onclick="event.stopPropagation();SMTool._onDot(' + node.id + ',\'progDelayer\',\'output\')" title="连线输出"></div>' +
            '</div>' +
            '<span class="scale-handle" onmousedown="event.stopPropagation();SMTool._onScaleStart(event,' + node.id + ')" title="拖拽缩放"><i class="scale-handle-icon"></i></span>';
    ...
    el.innerHTML =
        ...
        '<div class="header" onmousedown="event.stopPropagation();SMTool._onHD(event,' + node.id + ')">' +
            '<div class="header-titles">' +
                ...
                '<span class="name" ondblclick="SMTool._beginTrackNameEdit(event,' + node.id + ',this)">' + SMTool._esc(node._trackMode ? node._trackName : (node.currentAnim || node.name)) + '</span>' +
            '</div>' +
            '<div class="btns">' +
        ...
        '</div>' +
        // ---- 动画选择栏（轨道模式：隐藏下拉框，保留两端连线端点） ----
        (node._trackMode
            ? '<div class="anim-bar">' +
                '<div class="conn-dot input" onclick="event.stopPropagation();SMTool._onDot(' + node.id + ',\'' + SMTool._escAttr(node._trackName || '动画混合') + '\',\'input\')" title="连线输入"></div>' +
                '<span style="color:var(--accent);font-size:24px;font-weight:600;flex:1;text-align:center">🎬 轨道动画模式</span>' +
                '<div class="anim-progress-bar"></div>' +
                '<div class="conn-dot output" onclick="event.stopPropagation();SMTool._onDot(' + node.id + ',\'' + SMTool._escAttr(node._trackName || '动画混合') + '\',\'output\')" title="连线输出"></div>' +
              '</div>'
            : '<div class="anim-bar">' +
...
SMTool._delayerStep = function (nid, delta) {
    var node = SMData.nodes.get(nid);
    if (!node || (node.nodeType !== 'delayer' && node.nodeType !== 'progDelayer')) return;
...
SMTool._toggleLoop = function (nid) {
    ...
    if (node._trackMode && node._trackSequence && node._trackSequence.length > 0) {
        // 轨道动画的顶部按钮是整个轨道组合的总开关：单次时所有序列只跑一轮，
        // 循环时所有序列共同循环。绝不能调用旧 tracks 重建，否则画布会退回
        // 普通单动画表现，而浮窗仍显示真实轨道数据。
        for (var seqTi = 0; seqTi < node._trackSequence.length; seqTi++) {
            if (node._trackSequence[seqTi]) node._trackSequence[seqTi].loopSeq = node.loop;
        }
        if (node.state) SMTool._restartNodePlaybackAtZero(node);
        SMTool._refreshTrackPanel(node);
    } else {
        // 普通动画继续同步旧轨道 0。
        if (!node.tracks || node.tracks.length === 0) {
    ...
    // 当前浮窗属于该节点时与画布同时从第 0 帧重建，后续逐帧共享时间增量。
    var activePreview = SMData._animPreview;
    if (activePreview && activePreview.visible && activePreview.nodeId === nid &&
        !activePreview._layerSkeletons && typeof SMTool._initAnimPreview === 'function') {
        SMTool._initAnimPreview(node);
    }
...
SMTool._updateEl = function (node) {
    ...
    if (nameEl) {
        ...
        nameEl.ondblclick = function (event) { SMTool._beginTrackNameEdit(event, node.id, this); };
    }

    // ★ 轨道模式：anim-bar 保留连线端点，仅隐藏下拉框
    ...
    // 标题（轨道模式显示轨道名，普通模式显示原始英文动画名）
    var hn = el.querySelector('.header .name');
    if (hn) { var displayName = node._trackMode ? node._trackName : (node.currentAnim || node.name); hn.textContent = displayName; hn.title = displayName; }
...
SMTool._scheduleFloatLabelsUpdate = function () {
    if (SMTool._floatLabelsScheduled) return;
    SMTool._floatLabelsScheduled = true;
...
SMTool._renameTrackNode = function (node, rawName) {
    if (!node || !node._trackMode) return false;
    var nextName = String(rawName || '').trim() || '轨道动画';
    var oldName = node._trackName || '轨道动画';
    node._trackName = nextName;
    // 连线端口文本是该名称的引用，改名时同步迁移，不能留下失效的旧端口名。
    for (var i = 0; i < SMData.connections.length; i++) {
        var c = SMData.connections[i];
        if (c.fromNode === node.id && c.fromState === oldName) c.fromState = nextName;
        if (c.toNode === node.id && c.toState === oldName) c.toState = nextName;
    }
    SMTool._updateEl(node);
    SMTool._scheduleFloatLabelsUpdate();
    if (SMData._animPreview && SMData._animPreview.visible && SMData._animPreview.nodeId === node.id) {
        SMTool._updateAppTitle('🎬 ' + nextName, node.sourceFile || '');
    }
    if (typeof SMTool._buildLayerList === 'function') {
        var list = document.getElementById('appLayerList');
        if (list && list.style.display !== 'none') SMTool._buildLayerList();
    }
    if (typeof SMTool._updateFlowPanel === 'function') SMTool._updateFlowPanel();
    if (typeof SMTool._updateFullFlowPanel === 'function') {
...
SMTool._beginTrackNameEdit = function (event, nodeId, element) {
    var node = SMData.nodes.get(nodeId);
    if (!node || !node._trackMode || !element) return;
    ...
    var commit = function () {
        ...
        SMTool._renameTrackNode(node, element.innerText);
    };
    var onKey = function (e) {
        e.stopPropagation();
        if (e.key === 'Enter') { e.preventDefault(); element.blur(); }
        if (e.key === 'Escape') { element.textContent = node._trackName || '轨道动画'; element.blur(); }
    };
        ...
        while (!result.done) {
            ...
            if (!label) {
                ...
                nameSpan.addEventListener('blur', function() {
                    ...
                    if (newNode && newNode._trackMode) {
                        SMTool._renameTrackNode(newNode, this.innerText);
                    } else if (newNode) {
                        newNode.name = this.innerText.trim() || newNode.name;
                    }
                });
...
// ---- 更新选中状态 ----
SMTool._flowGraphSignature = function () {
    // 不包含节点 x/y：纯位置拖拽不会让流程拓扑缓存失效。
    var hash = 2166136261;
    function mix(value) {
    ...
    mix(SMData.nodes.size);
    SMData.nodes.forEach(function (node, id) {
        mix(id); mix(node.nodeType); mix(node.currentAnim); mix(node._trackName); mix(node.name);
    });
    mix(SMData.connections.length);
    for (var ci = 0; ci < SMData.connections.length; ci++) {
        var conn = SMData.connections[ci];
        mix(conn.id); mix(conn.fromNode); mix(conn.fromState); mix(conn.toNode); mix(conn.toState);
        mix(conn._layerNum); mix(conn.condition); mix(conn._mixDuration); mix(conn._hideLabel ? 1 : 0);
    }
    return (hash >>> 0).toString(36);
};

SMTool._flowPanelPassiveKey = function () {
    var selected = Array.from(SMData.selectedNodes || []).sort(function (a, b) { return a - b; }).join(',');
    return [SMData.flowMode, SMData.selectedNode || 0, selected, SMTool._flowGraphSignature()].join('|');
};

// 🔒🔒🔒 [LOCK-PERF-1] 实时核心与大列表展示必须永久解耦
// ⚠️ 解锁策略：仅用户明确说出「解锁 LOCK-PERF-1」才允许改变以下约束。
// 1. 当前选中流程、当前播放节点、Spine/轨道混合、浮窗和统一时钟必须立即执行，禁止等待 idle/rAF 列表任务。
// 2. 完整/三层动画流只同步生成首屏和当前 item；屏幕外后排 item 必须小批量、可取消地空闲加载。
// 3. 播放、节点拖拽、缩放、平移、拉线期间，后台流程列表装载必须暂停，绝不能抢占动画帧。
// 4. _playFullStep 等播放热路径禁止调用 _updateFlowPanel/_updateFullFlowPanel 或重建整张列表，只能轻量刷新当前 item。
// 5. 纯节点 x/y 变化不得使流程拓扑缓存失效；拖拽吸附热路径不得全量读取 DOM 布局。
// 后续任何人工或 AI 性能优化都应继续沿用“当前内容优先、后排数据渐进补齐”的机制。
SMTool._scheduleFlowPanelUpdate = function (options) {
    options = options || {};
    SMData._flowPanelDirty = true;
    var token = (SMData._flowPanelUpdateToken || 0) + 1;
    SMData._flowPanelUpdateToken = token;

    function queueAfterPaint(delay) {
        setTimeout(function () {
            var nextFrame = typeof requestAnimationFrame === 'function' ? requestAnimationFrame : function (callback) { setTimeout(callback, 0); };
            nextFrame(function () {
                if (token !== SMData._flowPanelUpdateToken) return;
                if (typeof requestIdleCallback === 'function') {
                    requestIdleCallback(runUpdate, { timeout: 220 });
                } else {
    ...
    function runUpdate() {
        if (token !== SMData._flowPanelUpdateToken) return;
        // 拖拽、平移、缩放和拉线期间绝不重建流程 item；等画布交互结束后再合并处理。
        if (SMData.draggedNode || SMData.isMultiDragging || SMData.isPanning || SMData.scalingNode ||
            SMData.connecting || SMData.draggingCP || SMData.draggingLabel ||
            (SMData._viewInteractingUntil && performance.now() < SMData._viewInteractingUntil)) {
            queueAfterPaint(90);
            return;
        }
        // 流程播放期间禁止完整列表重建；播放逻辑只走轻量的当前 item 刷新。
        if (SMData._fullPlayback && SMData._fullPlayback.isPlaying) {
            queueAfterPaint(160);
            return;
        }
        if (!SMData._flowPanel || !SMData._flowPanel.expanded) return;
        var passiveKey = SMTool._flowPanelPassiveKey();
        if (!options.force && passiveKey === SMData._lastPassiveFlowPanelKey) {
            SMData._flowPanelDirty = false;
            return;
        }
        SMTool._updateFlowPanel();
        SMData._lastPassiveFlowPanelKey = passiveKey;
        SMData._flowPanelDirty = false;

        if (SMData._flowPanelAutoSelectPending) {
            SMData._flowPanelAutoSelectPending = false;
            var pb = SMData._fullPlayback;
            var paths = SMData._fullPaths;
            if (SMData.flowMode === 'full' && pb.activePathIdx < 0 && paths && paths.length > 0 &&
                !pb.isPlaying && !SMData._flowExitLock) {
...
SMTool._updateSel = function () {
    ...
    if (SMData._flowFocus) {
        ...
        // 连线模式：高亮源节点及其已有的直接下游节点，未直连节点沿用现有置灰表现。
        // 这样从右端点拉线的当下就能看清“下一流程节点”，无需等缩放触发刷新。
        focusSet.add(SMData.connecting.nodeId);
        for (var cfi = 0; cfi < SMData.connections.length; cfi++) {
            var focusConn = SMData.connections[cfi];
            if (focusConn.fromNode === SMData.connecting.nodeId) focusSet.add(focusConn.toNode);
        }
    ...
    SMTool._commitFocusRevision(focusSet);
    ...
    SMTool._updateFloatPanel();
    // 当前选中流程属于高优先级：没有进行画布手势和播放时，立即生成首屏与当前 item。
    // 相同拓扑（尤其仅移动节点坐标）不刷新；后排 item 由面板内部空闲分批装载。
    var nextFlowPanelKey = SMTool._flowPanelPassiveKey();
    var flowGestureActive = SMData.draggedNode || SMData.isMultiDragging || SMData.isPanning || SMData.scalingNode ||
        SMData.connecting || SMData.draggingCP || SMData.draggingLabel;
    if (SMData._flowPanel && SMData._flowPanel.expanded && !flowGestureActive &&
        !(SMData._fullPlayback && SMData._fullPlayback.isPlaying) && nextFlowPanelKey !== SMData._lastPassiveFlowPanelKey) {
        SMTool._updateFlowPanel();
    } else {
        SMTool._scheduleFlowPanelUpdate();
    }
    ...
    if (typeof SMTool._scheduleNodeRectCacheWarmup === 'function') SMTool._scheduleNodeRectCacheWarmup();
};
...
// 统一维护浮窗暂停按钮的视觉状态。切换手动选中的动画节点时，
// 新预览始终从“正在播放”状态开始，不继承上一个浮窗的图标。
SMTool._setPreviewPauseUI = function (paused) {
    var btn = document.getElementById('appPauseBtn');
    if (!btn) return;
    btn.classList.toggle('paused', !!paused);
    btn.textContent = paused ? '▶' : '⏸';
    btn.title = paused ? '继续播放' : '暂停播放';
};

// 连线交互专用焦点：源节点 + 所有“从源节点右端直接流出”的下一节点。
// 同时选中源节点，确保从一个未选中节点的端点开始拉线时焦点不会被 _updateSel 清掉。
SMTool._commitFocusRevision = function (focusSet) {
    // 焦点内容变化不会改变 selectedNodes.size，旧渲染缓存因此无法察觉。
    // 用稳定签名驱动独立修订号，让节点/连线高亮在下一绘制帧立刻同步，而不再依赖缩放。
    var focusNodeSig = Array.from(focusSet || []).sort(function (a, b) { return a - b; }).join(',');
    var focusConnSig = '';
    if (SMData._flowFocus && SMData._flowFocus.connIds) {
        focusConnSig = Array.from(SMData._flowFocus.connIds).sort(function (a, b) { return a - b; }).join(',');
    }
    var nextFocusSignature = focusNodeSig + '|' + focusConnSig;
    if (nextFocusSignature === SMData._lastFocusSignature) return false;
    SMData._lastFocusSignature = nextFocusSignature;
    SMData._focusRevision = (SMData._focusRevision || 0) + 1;
    SMData._forceRedraw = true;
    return true;
};

SMTool._focusDirectSuccessors = function (sourceId) {
    if (!SMData.nodes.has(sourceId)) return null;
    if (typeof SMTool._setHighlightedTargetNode === 'function') SMTool._setHighlightedTargetNode(null);
    var nodeIds = new Set([sourceId]);
    var connIds = new Set();
    for (var i = 0; i < SMData.connections.length; i++) {
        var conn = SMData.connections[i];
        if (conn.fromNode !== sourceId) continue;
        nodeIds.add(conn.toNode);
        connIds.add(conn.id);
    }
    SMData.selectedNodes.clear();
    SMData.selectedNodes.add(sourceId);
    SMData.selectedNode = sourceId;
    SMData.selectedConnection = null;
    SMData._flowFocus = { nodeIds: nodeIds, connIds: connIds };
    return SMData._flowFocus;
};

// ---- 显示预览面板 ----
SMTool._showAnimPreview = function (node, restartAtZero, playbackOwner) {
    // ★ 层级节点：多层叠加预览
    if (node && node.nodeType === 'layer') {
        SMTool._showLayerPreview(node, playbackOwner);
        return;
    }
    // 跳过非 Spine 节点
    if (!node || node.nodeType !== 'spine') return;
    if (typeof SMTool._syncLayerListPreviewMode === 'function') SMTool._syncLayerListPreviewMode(false, false);
    // ★ 播放启动期间抑制中间态重建
    ...
    var previousPreviewNode = pp && pp.nodeId != null ? SMData.nodes.get(pp.nodeId) : null;
    if (playbackOwner && playbackOwner.type) {
        pp._playbackOwner = playbackOwner;
    } else {
        pp._playbackOwner = { type: 'single', nodeId: node.id, manualSelection: true };
        pp._flowFrozen = false;
        SMTool._setPreviewPauseUI(false);
    }

    // 手动切换到另一个独立节点时，先把关联画布节点也归零。浮窗随后从同一
    // 起点建立独立 Skeleton/State，避免刚打开时就继承两套不同的进度。
    if ((!playbackOwner || !playbackOwner.type) && (!pp.visible || pp.nodeId !== node.id) &&
        typeof SMTool._restartNodePlaybackAtZero === 'function') {
        SMTool._restartNodePlaybackAtZero(node);
    }
    ...
    if (pp.visible && pp.nodeId === node.id && pp.skeleton && pp.state) {
        if (restartAtZero) {
            SMTool._restartAnimPreviewStateAtZero(pp, node);
        } else if (pp.animName !== targetAnim) {
    ...
    if (sameSource) {
        var requestedMix = playbackOwner && playbackOwner.mixDurationSeconds;
        var mixedToNode = restartAtZero && requestedMix > 0 && previousPreviewNode &&
            SMTool._mixAnimPreviewToNode(pp, previousPreviewNode, node, requestedMix);
        pp.nodeId = node.id;
        // 流程切步时无条件归零；不能用“动画名相同”作为不重置的依据。
        if (restartAtZero && !mixedToNode) {
            SMTool._restartAnimPreviewStateAtZero(pp, node);
            SMTool._updateAppTitle('🎬 ' + (node._trackMode ? (node._trackName || '轨道动画') : targetAnim), node.sourceFile || '');
        } else if (mixedToNode) {
            SMTool._updateAppTitle('🎬 ' + (node._trackMode ? (node._trackName || '轨道动画') : targetAnim), node.sourceFile || '');
        } else if (node._trackMode && node._trackSequence && node._trackSequence.length > 0) {
            // ★ 轨道模式：重建序列链
            SMTool._updateAppTitle('🎬 ' + (node._trackName || '轨道动画'), node.sourceFile || '');
...
// 以真实运行状态解析浮窗归属。完整流程只要正在运行或暂停，就必须拥有
// 浮窗控制权；不能因异步重建浮窗时 owner 被短暂改写而退化成“重播当前节点”。
SMTool._resolvePreviewPlaybackOwner = function (pp) {
    var explicitOwner = pp && pp._playbackOwner;
    // 用户手动点选了新节点，该浮窗已明确脱离原流程上下文。
    // 与旧的“owner 被异步误写”情况区分：只有带 manualSelection 才优先。
    if (explicitOwner && explicitOwner.manualSelection) return explicitOwner;
    var fullPlayback = SMData._fullPlayback;
    var activePathIdx = fullPlayback ? fullPlayback.activePathIdx : -1;
    var activePath = activePathIdx >= 0 ? SMData._fullPaths[activePathIdx] : null;
    if (fullPlayback && (fullPlayback.isPlaying || fullPlayback._isPaused) &&
        activePath && activePath.nodes && activePath.nodes.length > 0) {
        var currentStep = Math.max(0, Math.min(fullPlayback.currentStep || 0, activePath.nodes.length - 1));
        return { type: 'flow', pathIdx: activePathIdx, nodeId: activePath.nodes[currentStep].id };
    }

    if (explicitOwner && explicitOwner.type === 'flow') {
        var explicitPath = SMData._fullPaths[explicitOwner.pathIdx];
        if (explicitPath && explicitPath.nodes && explicitPath.nodes.length > 0) return explicitOwner;
    }
    if (pp && pp._layerSkeletons) {
        return { type: 'parallel', nodeId: explicitOwner && explicitOwner.nodeId != null ?
            explicitOwner.nodeId : pp.nodeId };
    }
    return explicitOwner || { type: 'single', nodeId: pp ? pp.nodeId : null };
};
...
SMTool._restartPreview = function () {
    ...
    var owner = SMTool._resolvePreviewPlaybackOwner(pp);

    // 只有明确由完整流程创建的浮窗才重播整个流程，不能用“存在已选路径”猜测归属。
    var activeFlow = SMData._fullPlayback;
    if (owner.type === 'flow' && activeFlow && owner.pathIdx === activeFlow.activePathIdx &&
        SMData._fullPaths[owner.pathIdx] && SMData._fullPaths[owner.pathIdx].nodes.length > 0) {
    ...
    // ★ 并行预览：所有层经过同一个同步屏障归零，并立即绘制共同第 0 帧。
    if (owner.type === 'parallel' && pp._layerSkeletons) {
        SMTool._restartLayerPreviewCycle(pp, performance.now());
        return;
    }

    // ★ 独立单节点预览：清轨、setup pose、重建队列并同步覆盖为第 0 帧。
    if (pp.skeleton && pp.state && !pp._layerSkeletons) {
        var srcNode2 = SMData.nodes.get(owner.nodeId != null ? owner.nodeId : pp.nodeId);
        if (srcNode2) SMTool._restartAnimPreviewStateAtZero(pp, srcNode2);
    }
};
...
SMTool._togglePreviewPause = function () {
    ...
    var owner = SMTool._resolvePreviewPlaybackOwner(pp);
    var fullPlayback = SMData._fullPlayback;
    var isFlowOwner = owner.type === 'flow' && fullPlayback &&
        owner.pathIdx === fullPlayback.activePathIdx;
    var isPaused = isFlowOwner ? !!fullPlayback._isPaused : !!pp._flowFrozen;

    if (isPaused) {
        // ★ 继续播放：完整流保留当前步骤/时钟；独立或并行预览只解除本地冻结。
        if (isFlowOwner) {
        ...
        // ★ 暂停播放：完整流的逻辑时钟和画面一起冻结。
        if (isFlowOwner && fullPlayback.isPlaying) {
            SMTool._pauseFullPlayback();
        } else {
            pp._flowFrozen = true;
            SMTool._freezeAllNodes();
        }
        SMTool._setPreviewPauseUI(true);
    }
};
    ...
    while (!seResult.done) {
        ...
        if (el) {
            ...
            if (bar) {
                ...
                if (node && (node.nodeType === 'delayer' || node.nodeType === 'progDelayer' || node.nodeType === 'shortText' || node.nodeType === 'textBox' || node.nodeType === 'titleText' || node.nodeType === 'image')) {
...
SMTool._updateFlowPanel = function () {
    ...
    var inactiveHeaderControls = document.getElementById('flpHeaderControls');
    if (inactiveHeaderControls) inactiveHeaderControls.innerHTML = '';
    ...
    if (showFlow) {
        ...
        var selAnim = _disp(selNode.currentAnim, selNode) || (selNode.animations.length > 0 ? selNode.animations[0].name : selNode.name);

        // 只同步生成首屏组合，剩余组合在空闲帧分批加入。
        var chainTasks = [];
        if (upstreamConns.length > 0 && downstreamConns.length > 0) {
            for (var u = 0; u < upstreamConns.length; u++) {
                var upNode = SMData.nodes.get(upstreamConns[u].fromNode);
                if (!upNode) continue;
                for (var d = 0; d < downstreamConns.length; d++) {
                    var downNode = SMData.nodes.get(downstreamConns[d].toNode);
                    if (downNode) chainTasks.push({ upNode: upNode, upConn: upstreamConns[u], downNode: downNode, downConn: downstreamConns[d] });
                }
            }
        } else if (upstreamConns.length > 0) {
            for (var u2 = 0; u2 < upstreamConns.length; u2++) {
                var upNode2 = SMData.nodes.get(upstreamConns[u2].fromNode);
                if (upNode2) chainTasks.push({ upNode: upNode2, upConn: upstreamConns[u2] });
            }
        } else {
            for (var d2 = 0; d2 < downstreamConns.length; d2++) {
                var downNode2 = SMData.nodes.get(downstreamConns[d2].toNode);
                if (downNode2) chainTasks.push({ downNode: downNode2, downConn: downstreamConns[d2] });
            }
        }

        function _renderThreeChain(task) {
            var nodeIds = [];
            var connIds = [];
            var h = '<div class="flp-chain-group"><div class="flp-chain-row"';
            if (task.upNode) nodeIds.push(task.upNode.id);
            nodeIds.push(selNodeId);
            if (task.downNode) nodeIds.push(task.downNode.id);
            if (task.upConn) connIds.push(task.upConn.id);
            if (task.downConn) connIds.push(task.downConn.id);
            h += ' data-flow-nodes="' + nodeIds.join(',') + '" data-flow-conns="' + connIds.join(',') + '">';
            if (task.upNode) {
                var upAnim = _disp(task.upConn.fromState) || (task.upNode.currentAnim || (task.upNode.animations.length > 0 ? task.upNode.animations[0].name : task.upNode.name));
                var upCond = task.upConn.condition || '';
                h += '<span class="flp-node-l1" title="' + SMTool._esc(task.upNode.name) + '">' + SMTool._esc(upAnim) + '</span>';
                h += '<span class="flp-arrow">→</span>';
                h += upCond ? '<span class="flp-condition" data-flow-condition-conn="' + task.upConn.id + '" title="双击修改条件">条件：' + SMTool._esc(upCond) + '</span>' : '<span class="flp-condition-empty" data-flow-condition-conn="' + task.upConn.id + '" title="双击修改条件">条件：（空）</span>';
                h += '<span class="flp-arrow">→</span>';
            }
            h += '<span class="flp-node-l2" title="' + SMTool._esc(selNode.name) + '">' + SMTool._esc(selAnim) + '</span>';
            if (task.downNode) {
                var downAnim = _disp(task.downConn.toState) || (task.downNode.currentAnim || (task.downNode.animations.length > 0 ? task.downNode.animations[0].name : task.downNode.name));
                var downCond = task.downConn.condition || '';
                h += '<span class="flp-arrow">→</span>';
                h += downCond ? '<span class="flp-condition" data-flow-condition-conn="' + task.downConn.id + '" title="双击修改条件">条件：' + SMTool._esc(downCond) + '</span>' : '<span class="flp-condition-empty" data-flow-condition-conn="' + task.downConn.id + '" title="双击修改条件">条件：（空）</span>';
                h += '<span class="flp-arrow">→</span>';
                h += '<span class="flp-node-l3" title="' + SMTool._esc(task.downNode.name) + '">' + SMTool._esc(downAnim) + '</span>';
            }
            return h + '</div></div>';
        }

        var initialThreeCount = Math.min(chainTasks.length, SMData._flowPanel && SMData._flowPanel.maximized ? 12 : 6);
        var chainsHtml = '';
        for (var initialThreeIndex = 0; initialThreeIndex < initialThreeCount; initialThreeIndex++) {
            chainsHtml += _renderThreeChain(chainTasks[initialThreeIndex]);
        }
        ...
        var threeHydrationToken = (SMData._flowListHydrationToken || 0) + 1;
        SMData._flowListHydrationToken = threeHydrationToken;
        var nextThreeIndex = initialThreeCount;
        function _queueThreeHydration(delay) {
            if (nextThreeIndex >= chainTasks.length) return;
            setTimeout(function () {
                if (threeHydrationToken !== SMData._flowListHydrationToken) return;
                ...
                requestIdle(_hydrateThreeBatch, { timeout: 500 });
            }, delay || 0);
        }
        function _hydrateThreeBatch(deadline) {
            if (threeHydrationToken !== SMData._flowListHydrationToken || SMData.flowMode !== 'three' ||
                !SMData._flowPanel || !SMData._flowPanel.expanded || document.getElementById('flpContent') !== content) return;
            if ((SMData._fullPlayback && SMData._fullPlayback.isPlaying) || SMData.draggedNode || SMData.isMultiDragging ||
                SMData.isPanning || SMData.scalingNode || SMData.connecting || SMData.draggingCP || SMData.draggingLabel) {
                _queueThreeHydration(140);
                return;
            }
            var batchHtml = '';
            var count = 0;
            while (nextThreeIndex < chainTasks.length && count < 2 && (count === 0 || deadline.timeRemaining() > 3)) {
                batchHtml += _renderThreeChain(chainTasks[nextThreeIndex++]);
                count++;
            }
            if (batchHtml) content.insertAdjacentHTML('beforeend', batchHtml);
            if (nextThreeIndex < chainTasks.length) _queueThreeHydration(24);
        }
        _queueThreeHydration(80);
    } else if (SMData.selectedNodes.size > 1) {
        panel.classList.add('inactive');
...
// 完整动画流列表使用增量装载，但当前选中路径的交互必须立即可用。
// 这里统一绑定新插入的路径节点，避免每次刷新整张列表。
SMTool._bindFullFlowListElements = function (root) {
    if (!root) return;
    var pathRows = root.querySelectorAll('.flp-full-path');
    for (var ri = 0; ri < pathRows.length; ri++) {
        var row = pathRows[ri];
        if (row._fullFlowBound) continue;
        row._fullFlowBound = true;
        row.addEventListener('click', function (e) {
            e.stopPropagation();
            SMTool._selectFullPath(parseInt(this.getAttribute('data-path-idx')));
        });
    }

    var pathTitles = root.querySelectorAll('.flp-full-path-title');
    for (var pti = 0; pti < pathTitles.length; pti++) {
        var pathTitle = pathTitles[pti];
        if (pathTitle._fullFlowBound) continue;
        pathTitle._fullFlowBound = true;
        pathTitle.addEventListener('click', function (e) { e.stopPropagation(); });
        pathTitle.addEventListener('dblclick', function (e) {
            e.preventDefault();
            e.stopPropagation();
            ...
            var finish = function (cancelled) {
                ...
                var fallback = titleEl.getAttribute('data-default-title') || '动画';
                var nextTitle = cancelled ? original : (titleEl.innerText || '').trim();
                if (!nextTitle) nextTitle = fallback;
                titleEl.textContent = nextTitle;
                if (!cancelled) SMData._fullPathNames[decodeURIComponent(titleEl.getAttribute('data-path-key'))] = nextTitle;
            };
            var onBlur = function () { finish(false); };
            var onKeyDown = function (keyEvent) {
                keyEvent.stopPropagation();
                if (keyEvent.key === 'Enter') { keyEvent.preventDefault(); titleEl.blur(); }
                if (keyEvent.key === 'Escape') { keyEvent.preventDefault(); finish(true); }
            };
    ...
    var mixEditors = root.querySelectorAll('.flp-full-transition');
    for (var mei = 0; mei < mixEditors.length; mei++) {
        var editor = mixEditors[mei];
        if (editor._fullFlowBound) continue;
        editor._fullFlowBound = true;
        (function (boundEditor) {
            var connId = parseInt(boundEditor.getAttribute('data-conn-id'));
            var field = boundEditor.querySelector('input');
            boundEditor.addEventListener('click', function (e) {
                e.stopPropagation();
                var action = e.target && e.target.getAttribute('data-mix-action');
                if (action === 'decrease') field.value = SMTool._adjustConnectionMixDuration(connId, -0.1).toFixed(1);
                if (action === 'increase') field.value = SMTool._adjustConnectionMixDuration(connId, 0.1).toFixed(1);
            });
            boundEditor.addEventListener('mousedown', function (e) { e.stopPropagation(); });
            field.addEventListener('change', function () { this.value = SMTool._setConnectionMixDuration(connId, this.value).toFixed(1); });
            field.addEventListener('keydown', function (e) {
                e.stopPropagation();
                if (e.key === 'ArrowLeft') { e.preventDefault(); this.value = SMTool._adjustConnectionMixDuration(connId, -0.1).toFixed(1); }
                if (e.key === 'ArrowRight') { e.preventDefault(); this.value = SMTool._adjustConnectionMixDuration(connId, 0.1).toFixed(1); }
...
// 播放热路径只刷新当前路径的 class 和按钮，不重建任何流程 item。
SMTool._refreshFullFlowPanelRealtime = function () {
    if (SMData.flowMode !== 'full') return;
    var pb = SMData._fullPlayback;
    var list = document.querySelector('#flpContent .flp-full-list');
    if (list) {
        var rows = list.querySelectorAll('.flp-full-path');
        for (var ri = 0; ri < rows.length; ri++) {
            rows[ri].classList.toggle('active', parseInt(rows[ri].getAttribute('data-path-idx')) === pb.activePathIdx);
        }
        var activeRow = list.querySelector('.flp-full-path[data-path-idx="' + pb.activePathIdx + '"]');
        if (activeRow) {
            var states = activeRow.querySelectorAll('[data-flow-step-idx]');
            for (var si = 0; si < states.length; si++) {
                var stateEl = states[si];
                var stepIdx = parseInt(stateEl.getAttribute('data-flow-step-idx'));
                stateEl.classList.remove('played', 'current', 'upcoming');
                if (stateEl.classList.contains('cycle-close')) continue;
                if (stepIdx < pb.currentStep) stateEl.classList.add('played');
                else if (stepIdx === pb.currentStep) stateEl.classList.add('current');
                else stateEl.classList.add('upcoming');
            }
        }
    }
    ...
    var path = pb.activePathIdx >= 0 ? SMData._fullPaths[pb.activePathIdx] : null;
    var maxStep = path && path.nodes ? path.nodes.length : 0;
    if (path && path.nodes[maxStep - 1] && path.nodes[maxStep - 1].cycleClose) maxStep--;
    var layerComplete = false;
    if (path && pb.currentStep < maxStep) {
        var stepNode = path.nodes[pb.currentStep];
        layerComplete = !!(stepNode && stepNode._isLayerHub && SMData._animPreview && SMData._animPreview._allLayersCompletedOnce);
    }
    if (path && (pb.currentStep >= maxStep || layerComplete)) {
        playBtn.innerHTML = '🔄';
        playBtn.title = '重新播放';
    } else {
        playBtn.innerHTML = '▶';
        playBtn.title = !path ? '播放' : (pb.currentStep === maxStep - 1 ? '播放最后一节' : (pb.currentStep > 0 ? '继续播放' : '播放'));
    }
};

// Spine 状态与统一时钟先在当前调用栈完成；画布焦点和列表高亮随后更新，
// 防止非关键 DOM 操作占用动画切换的首帧。
SMTool._scheduleFullPlaybackVisualRefresh = function () {
    SMTool._refreshFullFlowPanelRealtime();
    var token = (SMData._fullPlaybackVisualToken || 0) + 1;
    SMData._fullPlaybackVisualToken = token;
    setTimeout(function () {
        if (token !== SMData._fullPlaybackVisualToken) return;
        SMTool._setFullComponentFocus(SMData.selectedNode);
        SMTool._updateSel();
        SMTool._updateStateRowColors();
    }, 0);
...
SMTool._updateFullFlowPanel = function (content, panel) {
    var _savedScrollTop = content.scrollTop;
    var headerControls = document.getElementById('flpHeaderControls');
    function _setHeaderControls(show) {
        if (!headerControls) return;
        headerControls.innerHTML = show ?
            '<div class="flp-full-controls">' +
                '<button class="flp-full-btn small" id="flpFullPrev" title="上一个状态">⏮</button>' +
                '<button class="flp-full-btn" id="flpFullPlay" title="播放">▶</button>' +
                '<button class="flp-full-btn small" id="flpFullNext" title="下一个状态">⏭</button>' +
            '</div>' : '';
    }
    var selNodeId = SMData.selectedNode;
    var showFlow = selNodeId && (
    ...
    panel.classList.remove('inactive');
    _setHeaderControls(true);

    // 辅助显示名（轨道模式显示轨道名）
    var _disp = function (s, node) {
        if (s === 'entry') return '入口';
        if (s === 'exit') return '出口';
        if (node && node._trackMode) return node._trackName || '轨道动画';
        return s;
    };
    ...
    if (!SMData._fullPathNames) SMData._fullPathNames = {};
    ...
    function _renderTransitionEditor(connection) {
        ...
        var mixValue = typeof SMTool._normalizeConnectionMixDuration === 'function' ?
            SMTool._normalizeConnectionMixDuration(connection._mixDuration) : Math.max(0, Number(connection._mixDuration) || 0);
        return '<span class="flp-full-transition" data-conn-id="' + connection.id + '">' +
            '<span class="flp-full-mix-editor">' +
                '<button type="button" data-mix-action="decrease" title="减少 0.1 秒">◀</button>' +
                '<span class="flp-full-mix-value"><input data-conn-mix-id="' + connection.id + '" type="number" min="0" step="0.1" value="' + mixValue.toFixed(1) + '" aria-label="混合过渡秒数"><span>s</span></span>' +
                '<button type="button" data-mix-action="increase" title="增加 0.1 秒">▶</button>' +
            '</span>' +
        '</span>';
    }
                ...
                for (var bni = 0; bni < br.nodes.length; bni++) {
                    ...
                    if (isNestedLayer) {
                        ...
                        if (nestedBranches && nestedBranches.length > 0) {
                            ...
                            h += '<span class="flp-full-state layer-hub-state" style="font-size:12px">📚 ' + SMTool._esc(_disp(bn.anim, SMData.nodes.get(bn.id))) + '</span>';
                            ...
                            h += '<span class="flp-full-state branch-state upcoming">📚 ' + SMTool._esc(_disp(bn.anim, SMData.nodes.get(bn.id))) + ' (无连线)</span>';
                        }
                    } else {
                        ...
                        h += '<span class="flp-full-state' + bState + '" data-br-layer="' + br.layer + '" data-br-idx="' + bni + '">' + SMTool._esc(_disp(bn.anim, SMData.nodes.get(bn.id))) + '</span>';
                    }
                    if (bni < br.nodes.length - 1) {
                        h += _renderTransitionEditor(_connectionBetween(bn.id, br.nodes[bni + 1].id));
                    }
                }
            }
            h += '</div>';
                ...
                for (var bni = 0; bni < br.nodes.length; bni++) {
                    ...
                    if (isNested2) {
                        ...
                        if (nestedBr2 && nestedBr2.length > 0) {
                            ...
                            h += '<span class="flp-full-state branch-state upcoming">📚 ' + SMTool._esc(_disp(bn.anim, SMData.nodes.get(bn.id))) + '</span>';
                        }
                    } else {
                        h += '<span class="flp-full-state branch-state upcoming">' + SMTool._esc(_disp(bn.anim, SMData.nodes.get(bn.id))) + '</span>';
                    }
        ...
        for (var li = 0; li < layerNums.length; li++) {
            ...
            for (var cj = 0; cj < conns.length; cj++) {
                ...
                for (var cni = 0; cni < chainIds.length; cni++) {
                    ...
                    } else if (chainNode.nodeType === 'progDelayer') {
                        animName = '⏱ ' + (chainNode._delayValue || 1) + 's';
                ...
                if (infoNode) {
                    ...
                    for (var cni2 = 0; cni2 < chain2.length; cni2++) {
                        ...
                        var an2 = cn2.nodeType === 'spine' ? (cn2.currentAnim || (cn2.animations && cn2.animations[0] ? cn2.animations[0].name : cn2.name)) :
                            ...
                            cn2.nodeType === 'progDelayer' ? '⏱ ' + (cn2._delayValue || 1) + 's' :
    ...
    function _renderFullPathItem(pi) {
        var path = paths[pi];
        ...
        itemHtml += '</div>'; // end .flp-full-path
        return itemHtml;
    }

    var html = '<div class="flp-full-layout">';
    // 只同步创建首屏与当前选中路径。后排 item 在空闲帧分批插入。
    html += '<div class="flp-full-list">';
    var initialCount = Math.min(paths.length, SMData._flowPanel && SMData._flowPanel.maximized ? 8 : 4);
    var renderedPathIndices = {};
    for (var pi = 0; pi < initialCount; pi++) {
        html += _renderFullPathItem(pi);
        renderedPathIndices[pi] = true;
    }
    if (activePathIdx >= initialCount && activePathIdx < paths.length) {
        html += _renderFullPathItem(activePathIdx);
        renderedPathIndices[activePathIdx] = true;
    }
    html += '</div>';
    ...
    SMTool._bindFullFlowListElements(newList);

    // 后排路径只在没有拖拽、缩放和流程播放时分批补齐。每批很小，且新一轮
    // 面板刷新会让旧任务自动失效，不允许后台列表工作抢占动画帧。
    var hydrationToken = (SMData._flowListHydrationToken || 0) + 1;
    SMData._flowListHydrationToken = hydrationToken;
    var nextHydrationIndex = initialCount;
    function _hasPendingFullPath() {
        while (nextHydrationIndex < paths.length && renderedPathIndices[nextHydrationIndex]) nextHydrationIndex++;
        return nextHydrationIndex < paths.length;
    }
    function _queueFullPathHydration(delay) {
        if (!_hasPendingFullPath()) return;
        setTimeout(function () {
            if (hydrationToken !== SMData._flowListHydrationToken) return;
            var requestIdle = typeof requestIdleCallback === 'function' ? requestIdleCallback : function (callback) {
                setTimeout(function () { callback({ timeRemaining: function () { return 8; }, didTimeout: false }); }, 0);
            };
            requestIdle(_hydrateFullPathBatch, { timeout: 500 });
        }, delay || 0);
    }
    function _hydrateFullPathBatch(deadline) {
        if (hydrationToken !== SMData._flowListHydrationToken || SMData.flowMode !== 'full' ||
            !SMData._flowPanel || !SMData._flowPanel.expanded ||
            document.querySelector('#flpContent .flp-full-list') !== newList) return;
        var playback = SMData._fullPlayback;
        if ((playback && playback.isPlaying) || SMData.draggedNode || SMData.isMultiDragging ||
            SMData.isPanning || SMData.scalingNode || SMData.connecting || SMData.draggingCP || SMData.draggingLabel) {
            _queueFullPathHydration(140);
            return;
        }
        var batchHtml = '';
        var batchCount = 0;
        while (_hasPendingFullPath() && batchCount < 1 && (batchCount === 0 || deadline.timeRemaining() > 3)) {
            batchHtml += _renderFullPathItem(nextHydrationIndex);
            renderedPathIndices[nextHydrationIndex] = true;
            nextHydrationIndex++;
            batchCount++;
        }
        if (batchHtml) {
            newList.insertAdjacentHTML('beforeend', batchHtml);
            SMTool._bindFullFlowListElements(newList);
        }
        if (_hasPendingFullPath()) _queueFullPathHydration(24);
    }
    _queueFullPathHydration(80);
...
SMTool._selectFullPath = function (pathIdx) {
    ...
    SMData._fullPlayback._clockStep = -1;
    SMData._fullPlayback._stepElapsed = 0;
    SMData._fullPlayback._stepDuration = 0;
    SMData._fullPlayback._clockMode = 'idle';
...
SMTool._startFullPlayback = function () {
    ...
    // 真正的“继续”不能重新应用当前步骤，否则会从当前节点第 0 帧重播。
    if (pb._isPaused && pb._clockStep === pb.currentStep) {
        SMTool._resumeFullPlayback();
        return;
    }
    // 如果已播完，从头开始
...
// 识别“最后节点重新连回源节点”的闭环，并提取 C→A 的真实混合参数。
SMTool._getClosedFlowCycleTransition = function (path) {
    if (!path || !path.nodes || path.nodes.length < 2) return null;
    var closeMarker = path.nodes[path.nodes.length - 1];
    var firstStep = path.nodes[0];
    var lastPlayable = path.nodes[path.nodes.length - 2];
    if (!closeMarker || !closeMarker.cycleClose || !firstStep || !lastPlayable || closeMarker.id !== firstStep.id) return null;
    return {
        fromNodeId: lastPlayable.id,
        toNodeId: firstStep.id,
        mixDurationSeconds: typeof SMTool._getConnectionMixDuration === 'function' ?
            SMTool._getConnectionMixDuration(lastPlayable.id, firstStep.id) : 0
    };
};

// 自然到达闭环边界时保留浮窗中 C 的末帧，并把 C→A 的混合信息交给第 0 步。
SMTool._restartFullPlaybackFromCycle = function (path) {
    var transition = SMTool._getClosedFlowCycleTransition(path);
    if (!transition) return false;
    return SMTool._restartFullPlaybackFromStart({ cycleTransition: transition });
};

// 从已选完整流程的第一个步骤重新开始。手动“重头播放”执行彻底初始化；
// 自然闭环则通过 options.cycleTransition 保留上一节点末帧以执行 C→A mixingFrom。
SMTool._restartFullPlaybackFromStart = function (options) {
    var pb = SMData._fullPlayback;
    if (!pb || pb.activePathIdx < 0) return false;
    var path = SMData._fullPaths[pb.activePathIdx];
    if (!path || !path.nodes || path.nodes.length === 0) return false;

    if (pb._timer) {
        clearTimeout(pb._timer);
        pb._timer = null;
    }
    var cycleTransition = options && options.cycleTransition ? options.cycleTransition : null;
    pb._cycleTransition = cycleTransition;
    pb._runToken = (pb._runToken || 0) + 1;
    pb.currentStep = 0;
    pb._stepped = false;
    pb._isPaused = false;
    pb.isPlaying = true;
    ...
    // 手动重播必须彻底初始化；自然闭环不能先清空浮窗中的 C 末帧，否则无法混合回 A。
    if (!cycleTransition) SMTool._resetAllToAnimFrame1();
    if (SMData._animPreview) {
        SMData._animPreview._flowFrozen = false;
        SMData._animPreview._allLayersCompletedOnce = false;
    }
    SMTool._playFullStep();
...
SMTool._pauseFullPlayback = function () {
    ...
    SMData._fullPlayback._runToken = (SMData._fullPlayback._runToken || 0) + 1;
    if (SMData._fullPlayback._timer) { clearTimeout(SMData._fullPlayback._timer); SMData._fullPlayback._timer = null; }
    // 冻结当前进度条（不清除，保持可见）
...
// 继续完整流程：保留当前步骤、TrackTime 和统一时钟累计值，不重新应用动画。
SMTool._resumeFullPlayback = function () {
    var pb = SMData._fullPlayback;
    if (!pb || pb.activePathIdx < 0 || !pb._isPaused) return false;
    var path = SMData._fullPaths[pb.activePathIdx];
    var stepNode = path && path.nodes ? path.nodes[pb.currentStep] : null;
    if (!stepNode || stepNode.cycleClose) return false;

    pb._runToken = (pb._runToken || 0) + 1;
    pb._isPaused = false;
    pb.isPlaying = true;
    var activeNode = SMData.nodes.get(stepNode.id);
    if (activeNode && activeNode.nodeType === 'layer') {
        if (SMData._animPreview) SMData._animPreview._flowFrozen = false;
        SMTool._freezeAllNodesExceptLayer(activeNode);
    } else if (activeNode && activeNode.nodeType === 'spine') {
        SMTool._pauseAllNodesExcept(stepNode.id);
        if (activeNode && activeNode.state) {
            for (var ti = 0; ti < 10; ti++) {
                var entry = activeNode.state.getCurrent(ti);
                if (entry) entry.timeScale = 1;
            }
        }
        if (SMData._animPreview) SMData._animPreview._flowFrozen = false;
    } else {
        // 延时器/隐藏器等逻辑步骤恢复时只恢复流程时钟，预览仍保持首帧或上一动画末帧。
        SMTool._pauseAllNodesExcept(stepNode.id);
        if (SMData._animPreview) SMData._animPreview._flowFrozen = true;
    }
    return true;
};
...
SMTool._resetAllToAnimFrame1 = function () {
    ...
    while (!r.done) {
        ...
        if (n.skeleton && n.state && n.skeletonData) {
            ...
            try {
                // 轨道模式必须重建全部原生轨道队列；普通模式应用节点自身动画第一帧。
                if (n._trackMode && n._trackSequence && n._trackSequence.length > 0) {
                    SMTool._applyTrackSequence(n);
                    n.skeleton.setToSetupPose();
                    for (var trackTi = 0; trackTi < 10; trackTi++) {
                        var trackEntry = n.state.getCurrent(trackTi);
                        if (trackEntry) trackEntry.trackTime = 0;
                    }
                    n.state.update(0);
                    n.state.apply(n.skeleton);
                    n.skeleton.updateWorldTransform(n._physParam);
                ...
                // 冻结所有已建立轨道在第一帧，等待流程轮到该节点。
                for (var freezeTi = 0; freezeTi < 10; freezeTi++) {
                    var freezeEntry = n.state.getCurrent(freezeTi);
                    if (freezeEntry) freezeEntry.timeScale = 0;
                }
            } catch (e) {}
        }
        r = nodesIter.next();
...
SMTool._applyStepToMainNode = function (stepNode) {
    ...
    if (spineNode && (spineNode.nodeType === 'delayer' || spineNode.nodeType === 'progDelayer' || spineNode.nodeType === 'hider' || spineNode.nodeType === 'loop')) {
        return spineNode;
    }
    if (spineNode && spineNode.state && spineNode.skeleton) {
        ...
        // ★ 轨道动画模式：使用序列链数据，不覆盖为单动画
        if (spineNode._trackMode && spineNode._trackSequence && spineNode._trackSequence.length > 0) {
            var trackHasLoopMode = spineNode.loop !== false &&
                !!(spineNode._loopMode || (spineNode._loopCount !== undefined && spineNode._loopCount !== 1));
            SMTool._applyTrackSequence(spineNode, { finitePass: !trackHasLoopMode });
        } else {
...
SMTool._goToNextStep = function () {
    ...
    if (stepNode && !stepNode.cycleClose) {
        ...
        if (nextSpineNode && nextSpineNode.nodeType === 'layer') {
            ...
            // ★ 触发右上角动画预览浮窗
            SMTool._showAnimPreview(nextSpineNode, true, {
                type: 'flow',
                pathIdx: pb.activePathIdx,
                nodeId: stepNode.id
            });
...
// 当流程源头是延时器/隐藏器等逻辑节点时，浮窗先显示本轮第一个可渲染节点的精确第 0 帧，
// 但保持冻结；逻辑节点结束后才由统一流程时钟真正启动该动画。
SMTool._primeFlowPreviewFromSource = function (path, owner) {
    if (!path || !path.nodes) return null;
    for (var i = 1; i < path.nodes.length; i++) {
        var candidateStep = path.nodes[i];
        if (!candidateStep || candidateStep.cycleClose) continue;
        var candidate = SMData.nodes.get(candidateStep.id);
        if (!candidate || (candidate.nodeType !== 'spine' && candidate.nodeType !== 'layer')) continue;
        var previewOwner = {
            type: 'flow',
            pathIdx: owner.pathIdx,
            nodeId: owner.nodeId,
            previewNodeId: candidate.id,
            previewOnly: true
        };
        if (candidate.nodeType === 'layer') {
            SMTool._showLayerPreview(candidate, previewOwner);
        } else {
            SMTool._showAnimPreview(candidate, true, previewOwner);
        }
        if (SMData._animPreview) SMData._animPreview._flowFrozen = true;
        return candidate;
    }
...
// 🔒 [LOCK-PERF-1] 播放当前步骤：此函数属于实时热路径，禁止在内部重建完整/三层流程列表。
SMTool._playFullStep = function () {
    ...
    if (!path || pb.currentStep >= path.nodes.length) {
        if (path && path.nodes && path.nodes.length > 0) SMTool._restartFullPlaybackFromStart();
        return;
    }
    ...
    if (pb.currentStep > 0 && typeof SMTool._getConnectionMixDuration === 'function') {
        var previousStep = path.nodes[pb.currentStep - 1];
        if (previousStep) flowOwner.mixDurationSeconds = SMTool._getConnectionMixDuration(previousStep.id, stepNode.id);
    }
    if (pb.currentStep === 0 && pb._cycleTransition && pb._cycleTransition.toNodeId === stepNode.id) {
        flowOwner.mixDurationSeconds = pb._cycleTransition.mixDurationSeconds || 0;
        flowOwner.cycleFromNodeId = pb._cycleTransition.fromNodeId;
        pb._cycleTransition = null;
    }

    // 闭环节点是可视化标记，不是一个独立播放周期；到达它等价于整条流程完成。
    if (stepNode.cycleClose) {
        if (!SMTool._restartFullPlaybackFromCycle(path)) SMTool._restartFullPlaybackFromStart();
        return;
    }
    ...
    if (spineNode && spineNode.nodeType === 'layer') {
        pb._clockStep = pb.currentStep;
        pb._stepElapsed = 0;
        pb._stepDuration = Infinity;
        pb._clockMode = 'parallel-barrier';
        SMTool._showLayerPreview(spineNode, flowOwner);
        SMTool._scheduleFullPlaybackVisualRefresh();
    ...
    // 主节点已经清轨、归零并应用当前步骤后，再重建浮窗。
    // 顺序不能反过来，否则流程大循环时浮窗可能读取上一轮的 currentAnim/TrackTime。
    if (spineNode && spineNode.nodeType === 'spine') {
        SMTool._showAnimPreview(spineNode, true, flowOwner);
    } else if (SMData._animPreview) {
        // 逻辑步骤不允许旧浮窗继续自走；源头逻辑步骤先呈现后续首个动画的第 0 帧。
        SMData._animPreview._playbackOwner = flowOwner;
        if (pb.currentStep === 0) SMTool._primeFlowPreviewFromSource(path, flowOwner);
        SMData._animPreview._flowFrozen = true;
    }

    // 列表和画布焦点不参与播放计时的关键路径，统一时钟初始化后再轻量刷新。
    ...
    } else if (spineNode && spineNode.nodeType === 'progDelayer') {
        duration = (spineNode._delayValue || 1.0) * 1000;
    ...
    // 延时器进度条（使用计算后的总时长）
    if (spineNode && (spineNode.nodeType === 'delayer' || spineNode.nodeType === 'progDelayer')) {
    ...
    // 步骤推进只由渲染循环中的统一时钟负责，不再创建节点级 setTimeout。
    pb._timer = null;
    pb._clockStep = pb.currentStep;
    pb._stepElapsed = 0;
    pb._stepDuration = Math.max(0, timerDelay / 1000);
    pb._clockMode = 'timed-step';
    SMTool._scheduleFullPlaybackVisualRefresh();
};

// 完整流程唯一时钟。dt 与主画布、浮窗使用同一帧增量，因此不会出现
// wall-clock 定时器已经切步、Spine 画面仍停在中间帧的竞争。
SMTool._tickFullPlayback = function (dt) {
    var pb = SMData._fullPlayback;
    if (!pb || !pb.isPlaying || pb.activePathIdx < 0) return;
    if (pb._clockStep !== pb.currentStep || pb._clockMode !== 'timed-step') return;
    if (!isFinite(pb._stepDuration)) return;

    var safeDt = Number(dt);
    if (!isFinite(safeDt) || safeDt < 0) safeDt = 0;
    pb._stepElapsed = (pb._stepElapsed || 0) + safeDt;
    if (pb._stepElapsed + 0.000001 < pb._stepDuration) return;

    var path = SMData._fullPaths[pb.activePathIdx];
    if (!path || !path.nodes || path.nodes.length === 0) return;
    pb.currentStep++;
    var nextStep = path.nodes[pb.currentStep];
    if (pb.currentStep >= path.nodes.length || !nextStep || nextStep.cycleClose) {
        if (!SMTool._restartFullPlaybackFromCycle(path)) SMTool._restartFullPlaybackFromStart();
    } else {
        SMTool._playFullStep();
    }
